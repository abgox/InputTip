...
fn_symbol_pos(*) {
    ...
    symbolPos(info) {
        ...
        tab.UseTab(2)
        g.AddEdit("ReadOnly VScroll r7 w" w, "1. 简要说明`n   - 在 v1 版本中，通过在鼠标的附近显示符号，没有发现兼容性问题`n   - 而 v2 版本中，部分窗口无法获取到输入光标位置，就可以使用它`n   - 它会让符号显示在鼠标附近，并跟随鼠标移动`n   - 详情参考下方的相关链接`n`n2. 配置项 —— 在鼠标附近显示符号的应用窗口`n   - 【指定窗口】: 还需要通过下方的按钮去指定窗口`n   - 【所有窗口】: 它就实现了 v1 版本中同样的效果`n`n3. 配置项 —— 在鼠标附近显示时的水平偏移量`n`n4. 配置项 —— 在鼠标附近显示时的垂直偏移量`n`n5. 按钮 —— 指定在鼠标附近显示符号的应用窗口`n   - 如果你使用了【指定窗口】，就需要使用它`n   - 通过它去指定需要在鼠标附近显示符号的应用窗口`n`n6. 建议`n   - 减小【更改配置】的【轮询响应间隔】，保持在 50 毫秒以内`n   - 因为显示在鼠标附近时，符号位置会随鼠标移动实时更新`n   - 如果此值过高，符号位置更新不及时，会出现符号显示卡顿的现象")
