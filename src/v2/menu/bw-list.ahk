fn_bw_list(*) {
    ...
    bwListGui(info) {
        ...
        g.AddLink("cGray", '如果你正在使用 <a href="https://inputtip.pages.dev/FAQ/white-list">白名单机制</a>，InputTip 提供了同步添加机制`n当使用以下配置菜单时，添加的应用进程会同步添加到「白」名单中`n「设置光标获取模式」「设置特殊偏移量」「指定窗口自动切换状态」')
        ...
        e_change_list(item, *) {
            if (useWhiteList = item.value) {
                ...
                warningGui(info) {
                    ...
                    _g.AddButton("w" bw, "【否】不，我只是不小心点到了").OnEvent("Click", e_no)
                    e_yes(*) {
                        _g.Destroy()
                        gc._bw_list.Value := 1
