...
fn_check_update(*) {
    ...
    checkUpdateGui(info) {
        ...
        aboutText := '1. 配置项 —— 每隔多少分钟检查一次版本更新`n   - 单位: 分钟，默认 1440 分钟(1 天)`n   - 避免程序错误，可以设置的最大范围是 0-50000 分钟`n   - 如果为 0，则表示不检查版本更新`n   - 如果不为 0，在 InputTip 启动时，会立即检查一次`n   - 如果大于 50000，则生效的值是 50000`n`n2. 配置项 —— 是否启用自动静默更新`n   - 前提条件:【每隔多少分钟检查一次版本更新】的值不为 0`n   - 启用后，不再弹出更新弹框，而是利用空闲时间自动更新`n   - 空闲时间: 3 分钟内没有鼠标或键盘操作`n`n3. 按钮 —— 立即检查版本更新`n   - 点击这个按钮后，会立即检查一次版本更新`n   - 如果没有更新弹窗且不是网络问题，则表示当前就是最新版本'
        lineN := "10"
        if (!A_IsCompiled) {
