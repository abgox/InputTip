fn_check_update(*) {
    ...
    fn(x, y, w, h) {
        ...
        g := Gui("AlwaysOnTop", "InputTip - 设置更新检查")
        g.SetFont(fz, "微软雅黑")
        g.AddEdit("ReadOnly cRed -VScroll w" w - g.MarginX * 2, "- 单位: 分钟，默认 1440 分钟(1 天)`n- 避免程序错误，可以设置的最大范围是 0-50000 分钟`n- 如果为 0，则表示不检查版本更新`n- 如果不为 0，在 InputTip 启动后，会立即检查一次`n- 如果大于 50000，则会直接使用 50000`n")
