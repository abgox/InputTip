...
- 需要注意:
  ...
  - 你可以通过 `光标获取模式` 明确指定应用使用哪种模式
    - 如果获取不到:
      - 符号无法显示
      - 这就只能使用 [鼠标样式方案](#鼠标样式方案) 或者 `在鼠标附近显示符号`
...
> - 你可以使用 `在鼠标附近显示符号` 来解决此问题
> - 以 `WPS` 为例
>   - 使用 `在鼠标附近显示符号`，根据窗口提示，将它添加到其中，即可实现在鼠标附近显示
...
- **【通用】**
  ...
  - [手心输入法](https://www.xinshuru.com/): 使用 `Shift` 切换中英文状态无法正常识别，需要参照下方的使用方式
    ...
    - 其次，修改 InputTip 的配置
      - `托盘菜单` => `输入法模式` => `指定内部实现切换输入法状态的方式`，选择 `模拟输入 Ctrl + Space`
