fn_startup(item, *) {
    ...
    if (isStartUp) {
        ...
        startupGui(info) {
            ...
            g.AddEdit("xs ReadOnly -VScroll cGray w" bw, "1. 下方有多种方式设置开机自启动，请选择有效的方式`n2. 推荐「任务计划程序」或「注册表」，应用快捷方式可能无效`n3. 如果你不希望每次启动都弹出管理员授权(UAC)窗口:`n   - 将系统设置中的「更改用户账户控制设置」修改为【从不通知】`n   - 或者直接使用下方的「任务计划程序」设置开机自启动")
