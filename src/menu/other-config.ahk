fn_ohter_config(*) {
    ...
    ohterConfigGui(info) {
        ...
        e_enableJABSupport(item, *) {
            ...
            if (enableJABSupport) {
                ...
                JABGui(info) {
                    ...
                    g.AddEdit("xs -VScroll ReadOnly cGray w" w, "1. 启用 Java Access Bridge (如果不需要符号显示，忽略此步骤)`n2. 点击下方的或【托盘菜单】中的【光标获取模式】`n3. 设置 JetBrains IDE 或其他 JAB 应用进程的光标获取模式为【JAB】`n4. 如果未生效，请重启 InputTip`n5. 如果仍未生效，请重启正在运行的 JetBrains IDE 或其他 JAB 应用`n6. 如果仍未生效，请重启系统`n7. 有多块屏幕时，副屏幕上可能有坐标偏差，需要通过【特殊偏移量】调整")
        ...
        g.AddText("Section xs", "5. 按键次数统计的文字模板")
        _ := g.AddEdit("w" bw)
