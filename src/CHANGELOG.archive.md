...
- 在 `输入法相关` 中添加 `是否将输入法状态导出`
  ...
  - 它会写入 `CN`、`EN`、`Caps`
  - 这让其他脚本或程序和 InputTip 集成变得更加容易，例如 [InputTip for VSCode](https://inputtip.abgox.com/docs/inputtip-for-vscode)
- 其他的修复和优化
...
- 整合 `符号显示黑白名单`
  ...
  - 如果想要显示符号，必须设置 `白名单`
  - 详情参考: [符号显示方案的白名单机制](https://inputtip.abgox.com/docs/symbol/list-mechanism)
...
- `firefox.exe`(火狐浏览器) 默认的光标获取模式从 `GUI_UIA` 修改为 `MSAA`
  - https://inputtip.abgox.com/docs/symbol/app-compatibility
- 其他的修复和优化
