...
fn_cursor_mode(*) {
    ...
    showGui(deep := 0) {
        ...
        modeGui(info) {
            ...
            tab.UseTab(2)
            g.AddEdit("ReadOnly VScroll r11 w" w, "1. 简要说明`n   - 这个菜单用来设置【设置光标获取模式】的匹配规则`n   - 下方是对应的规则列表`n`n2. 规则列表 —— 进程名称`n   - 窗口实际的应用进程名称`n`n3. 规则列表 —— 光标获取模式`n   - 包括【HOOK】【UIA】【MSAA】【JAB】等等`n   - 不用在意这些模式是啥，哪个能用，就用哪个即可`n   - 如果想了解详细的相关内容，可以查看下方相关链接`n`n4. 规则列表 —— 创建时间`n   - 它是每条规则的创建时间`n`n5. 光标获取模式 —— JAB`n   - 在所有的光标获取模式中，【JAB】是特殊的`n   - 它仅用于需要使用 Java Access Bridge 的进程，比如 JetBrains IDE`n   - 它需要配合【托盘菜单】中的【启用 JAB/JetBrains IDE 支持】一起使用`n   - 详情参考【启用 JAB/JetBrains IDE 支持】的使用说明")
