...
- 关于 `UAC` 权限提示窗口
  ...
  - 由于软件需要管理员权限才能正常运行，所以会弹出 `UAC` 权限提示窗口
  - 如果你希望不弹出此窗口，你可以尝试以下方法
    - 在 `系统设置`中，打开 `更改用户账户控制设置`，将其设置为 `永不通知`
    - 使用 [UAC 白名单小工具](https://github.com/XIU2/UACWhitelistTool)
      - 需要注意，如果使用它的话，你就不应该勾选 `InputTip.exe` 托盘菜单中的 `开机自启动` 选项
      - 你应该按下 `Win` + `R` 打开 `运行`，在 `运行` 中输入 `shell:startup`，然后将 [UAC 白名单小工具](https://github.com/XIU2/UACWhitelistTool) 生成的快捷方式 `noUAC.InputTip` 放入弹出来的开机自启动目录中
...
- 当 `托盘菜单` 中 `更改配置` => `显示形式` => `是否显示图片符号` 设置为 `1` 时，会在输入光标处显示对应的图片符号
...
- 当 `托盘菜单` 中 `更改配置` => `显示形式` => `是否显示图片符号` 设置为 `1` 时，同时 `是否显示图片符号` 和 `是否显示文本符号` 都为 `0` 时，会在输入光标处显示不同颜色的方块符号
...
- 当 `托盘菜单` 中 `更改配置` => `显示形式` => `是否显示文本符号` 设置为 `1` 时，同时 `是否显示方块符号` 为 `1`、`是否显示图片符号` 为 `0` 时，会在输入光标处显示对应的文本符号
