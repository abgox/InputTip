...
fn_ohter_config(*) {
    ...
    ohterConfigGui(info) {
        ...
        tab.UseTab(3)
        g.AddEdit("Section r15 ReadOnly w" w, "1. 按钮 —— 更新检查`n   - 点击后，会打开版本更新相关的配置菜单`n`n2. 按钮 —— JAB/JetBrains IDE 支持`n   - 如果你正在使用 JetBrains IDE，请双击启用它，并根据窗口提示完成所有步骤`n   - 完成后，InputTip 才能在 JetBrains IDE 中正确获取到输入光标位置并显示符号`n`n3. 按钮 —— 打开软件目录`n   - 点击后，会自动打开软件所在的运行目录`n`n4. 按钮 —— 设置用户名`n   - InputTip 需要用到你的用户名来确保【开机自启动】等功能正常运行`n   - 首次初始化时会自动获取用户名并保存到配置文件中，之后可通过此按钮修改`n   - 对于域用户，自动获取的用户名缺少域前缀，需要手动添加`n`n5. 配置 —— 自定义软件托盘图标`n   - 只需要将 png 图片添加到图片目录中，然后在下拉列表中选择它即可实现自定义`n`n6. 按钮 —— 打开图片目录`n   - 点击它后，会自动打开图片目录，便于你完成【自定义软件托盘图标】`n`n7. 按钮 —— 刷新下拉列表`n   - 当你将 png 图片添加到图片目录后，可能需要通过点击它刷新下拉列表`n`n8. 配置 —— 配置菜单字体大小`n   - 可以通过设置字体大小来优化配置菜单在不同屏幕上的显示效果`n   - 取值范围: 5-30，建议 12-20 之间`n`n9. 配置 —— 轮询响应间隔时间`n   - 每隔 x 毫秒，InputTip 就会更新一次符号的位置/状态、输入法状态、鼠标样式等`n   - 这里的 x 就是轮询响应间隔时间，单位：毫秒，默认为 20`n   - 建议 50 以内，因为轮询间隔越短，依赖轮询的部分功能的稳定性越好`n`n10. 配置 —— 按键输入次数统计`n   - 开启后，InputTip 会记录你的按键输入次数`n   - 然后通过写入下方的文字模板中进行显示`n   - 注意: 只有当上一次按键和当前按键不同时，才会记为一次有效按键`n`n11. 配置 —— 鼠标悬停在【托盘图标】上时的文字模板`n   - 当鼠标悬停在【托盘图标】上时，会根据此处设置的文字模板进行显示`n   - 模板变量: %\n% (换行)，%keyCount% (按键次数)，%appState% (软件运行状态)`n`n12. 配置 —— 按键输入次数统计的文字模板`n   - 当开启了【按键输入次数统计】后，此处的文字模板会被添加到鼠标悬停的提示中`n   - 模板变量: %\n% (换行)，%keyCount% (按键次数)，%appState% (软件运行状态)")
