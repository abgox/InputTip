...
fn_input_mode(*) {
    ...
    inputModeGui(info) {
        ...
        tab.UseTab(4)
        g.AddEdit("Section r13 ReadOnly w" w, "1. 配置项 —— 指定内部实现切换输入法状态的方式`n   - InputTip 中的以下功能需要 InputTip 来切换输入法状态`n      -【设置状态切换快捷键】`n      -【指定窗口自动切换状态】`n   - 比如通过【设置状态切换快捷键】设置了 LShift 来切换英文状态`n      - LShift 就是键盘左边的 Shift 键，以此类推`n      - 当按下 LShift 后，InputTip 会读取当前的输入法状态`n      - 如果已经是英文状态，则保持英文状态`n      - 否则 InputTip 需要去切换输入法状态到中文状态`n   - 配置项有以下可选值，只建议使用 LShift 或 RShift`n      - 模拟输入 LShift`n      - 模拟输入 RShift`n      - 模拟输入 Ctrl+Space`n      - 内部调用 DLL`n`n2. 可选值 —— 模拟输入 LShift`n   - 当需要切换输入法状态时，InputTip 会模拟输入一个 LShift 键`n   - 这相当于 InputTip 帮你按了一次键盘左边的 Shift 键`n   - 只要使用的输入法可以通过左边的 Shift 键切换输入法状态，就可以使用它`n`n3. 可选值 —— 模拟输入 RShift`n   - 和 LShift 类似，只是会模拟输入键盘右边的 Shift 键`n`n4. 可选值 —— 模拟输入 Ctrl+Space`n   - 和 LShift、RShift 类似，只是会模拟输入组合键 Ctrl + Space (空格)`n   - 如果 LShift、RShift 是可用的，就不建议使用它，它可能导致一些按键冲突`n`n5. 可选值 —— 内部调用 DLL`n   - 不建议使用它，除非是特殊情况，其他几种都无法正常使用`n   - 当需要切换输入法状态时，InputTip 会直接调用系统 DLL 接口去设置输入法状态`n   - 这种方式是一种强行设置行为，可能会导致以下问题，不建议使用`n      - 对于部分输入法可能无效`n      - DLL 调用行为有概率对输入法本身产生副作用`n      - 当需要设置为中文状态时，没有中文状态的键盘(如: 美式键盘)，也会被强行设置`n          - 这会导致基于此的鼠标样式和符号出现暂时的显示错误")
