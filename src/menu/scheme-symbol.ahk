...
fn_scheme_symbol(*) {
    ...
    symbolStyleGui(info) {
        ...
        tab.UseTab(2)
        g.AddEdit("ReadOnly r14 w" bw, "1. 配置 —— 指定符号的类型`n   - 如果要显示符号，还必须添加【符号的白名单】`n   - 只有在【符号的白名单】中的应用进程，InputTip 才会尝试显示符号`n   - 它会先尝试获取输入光标位置，如果获取失败，则不显示符号`n   - 可以尝试【在鼠标附近显示符号】作为这种失败情况的折中方案`n`n2. 配置 —— 符号的延时隐藏`n   - 它指的是当无键盘和鼠标左键点击操作时，符号在多久后隐藏`n   - 单位: 毫秒，默认为 0，表示永不隐藏`n   - 当符号隐藏后，下次键盘操作或点击鼠标左键时再次显示`n`n3. 配置 —— 鼠标悬停时隐藏`n   - 它指的是当鼠标悬停在符号上时，符号是否需要隐藏`n   - 当符号隐藏后，下次键盘操作或光标位置变化时再次显示`n`n4. 配置 —— 偏移量参考原点`n   - 输入光标附近显示的符号的垂直偏移量会基于这个参考原点`n   - 如果你希望符号显示在文字的下方时，使用【输入光标下方】会更好`n   - 否则，请保持默认的【输入光标上方】`n   - 这个配置项对 JAB/JetBrains IDE 程序无效，需使用【特殊偏移量】单独处理`n`n5. 按钮 —— 图片符号/方块符号/文本符号`n   - 点击后，打开对应的符号配置菜单")
